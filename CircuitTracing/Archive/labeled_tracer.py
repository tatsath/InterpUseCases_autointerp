"""
Labeled Circuit Tracer

Shows feature numbers with their semantic labels from the finetuning analysis.
"""
...
class LabeledTracer:
    def __init__(self, model_path, sae_path):
        ...
        print(f"Loaded labels for {len(self.feature_labels)} features")
    
    def _load_feature_labels(self):
        """Load feature labels from the finetuning analysis results."""
        labels = {}
        
        # Layer 4 labels (from README analysis)
        labels[4] = {
    ...
    def find_connections(self, results):
        ...
        for conn in connections[:10]:
            # Get label from the first layer where it appears
...
def main():
    ...
    tracer = LabeledTracer(model_path, sae_path)
    ...
    # Most common features with labels
    feature_counts = {}
