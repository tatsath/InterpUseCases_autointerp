...
# Sparse AutoInterp Full Analysis - Layer 4
# This script runs AutoInterp Full analysis on features 10-20 of layer 4 for sparse autoencoder
# Usage: ./run_sparse_layer4_full_analysis.sh

echo "🔍 Sparse AutoInterp Full Analysis - Layer 4 (70B Qwen)"
echo "========================================================="
...
echo "  • Explainer Model: Qwen2.5-72B-Instruct (70B parameters)"
...
RESULTS_DIR="sparse_layer4_full_results_70b_qwen"
mkdir -p "$RESULTS_DIR"
...
# Set environment variables for better performance (adjusted for memory constraints)
...
# Create run name
RUN_NAME="sparse_layer4_full_70b_qwen"
...
cd ../../autointerp/autointerp_full
if [ -d "results/sparse_layer4_full" ]; then
    rm -rf "results/sparse_layer4_full"
    echo "🗑️  Cleaned up temporary results"
fi
cd ../../InterpUseCases_autointerp/FinanceLabeling

echo "🎯 Sparse AutoInterp Full Analysis Summary"
echo "========================================="
echo "📊 Results saved in: $RESULTS_DIR/"

# Check final status based on actual results
if [ -d "$RESULTS_DIR/sparse_layer4_full" ] && [ -d "$RESULTS_DIR/sparse_layer4_full/explanations" ]; then
    FEATURE_COUNT=$(find "$RESULTS_DIR/sparse_layer4_full/explanations" -name "feature_*.json" 2>/dev/null | wc -l)
    if [ $FEATURE_COUNT -gt 0 ]; then
        echo "   ✅ sparse_layer4_full/ (SUCCESS - $FEATURE_COUNT features analyzed)"
...
latents_dir = 'sparse_layer4_full_results/sparse_layer4_full/latents'
log_dir = 'sparse_layer4_full_results/sparse_layer4_full/log'
...
"
        ...
        echo "   ❌ sparse_layer4_full/ (FAILED - no feature explanations found)"
        echo ""
    ...
    echo "   ❌ sparse_layer4_full/ (FAILED - no results directory found)"
    echo ""
