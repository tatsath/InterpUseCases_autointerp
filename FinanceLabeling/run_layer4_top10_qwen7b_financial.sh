...
# Layer 4 Top 10 Features Analysis with Qwen 7B Model + Financial Dataset
# This script runs AutoInterp Full analysis on the top 10 features from layer 4
# using Qwen 7B model with the same financial dataset as previous analysis
# Usage: ./run_layer4_top10_qwen7b_financial.sh

echo "🚀 Layer 4 Top 10 Features Analysis - Qwen 7B Model + Financial Dataset"
echo "====================================================================="
echo "🔍 Running detailed analysis on top 10 features from layer 4:"
echo "  • Layer: 4"
echo "  • Top 10 features (from previous analysis)"
echo "  • Qwen 7B model for faster processing"
...
EXPLAINER_MODEL="Qwen/Qwen2.5-7B-Instruct"  # Using the 7B model instead of 72B
...
RESULTS_DIR="Test_results3"
mkdir -p "$RESULTS_DIR"

echo "📁 Results will be saved to: $RESULTS_DIR/"
echo "🎯 Features to analyze: $FEATURES"
echo "🤖 Using Qwen 7B model: $EXPLAINER_MODEL"
...
# Set environment variables for better performance (optimized for 7B model)
export VLLM_ALLOW_LONG_MAX_MODEL_LEN=1
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
export CUDA_VISIBLE_DEVICES="1,2,3,4"  # Use GPUs 1-4 to avoid processes on GPU 0
export VLLM_USE_DEEP_GEMM=1
export VLLM_GPU_MEMORY_UTILIZATION=0.8  # Higher utilization for smaller model
export VLLM_MAX_MODEL_LEN=8192  # Increased for longer contexts
export VLLM_BLOCK_SIZE=32
export VLLM_SWAP_SPACE=0

# Run AutoInterp Full for layer 4 with Qwen 7B model and financial dataset
echo "🔍 Analyzing Layer $LAYER with AutoInterp Full (Qwen 7B Model + Financial Dataset)..."
echo "----------------------------------------"

cd ../../autointerp/autointerp_full

# Create run name
RUN_NAME="layer4_top10_qwen7b_financial"
    ...
    if [ -d "results/$RUN_NAME" ]; then
        ...
        if [ -d "../../InterpUseCases_autointerp/FinanceLabeling/$RESULTS_DIR/$RUN_NAME/explanations" ] && [ "$(ls -A ../../InterpUseCases_autointerp/FinanceLabeling/$RESULTS_DIR/$RUN_NAME/explanations)" ]; then
            ...
            if [ -f "../../InterpUseCases_autointerp/FinanceLabeling/$RESULTS_DIR/$RUN_NAME/results_summary.csv" ]; then
                cp "../../InterpUseCases_autointerp/FinanceLabeling/$RESULTS_DIR/$RUN_NAME/results_summary.csv" "../../InterpUseCases_autointerp/FinanceLabeling/$RESULTS_DIR/results_summary_layer${LAYER}_qwen7b_financial.csv"
                echo "📈 Summary saved: FinanceLabeling/$RESULTS_DIR/results_summary_layer${LAYER}_qwen7b_financial.csv"
            fi
        fi
        
        # Verify that we actually have feature explanations
        FEATURE_COUNT=$(find "../../InterpUseCases_autointerp/FinanceLabeling/$RESULTS_DIR/$RUN_NAME/explanations" -name "layers.4_latent*.txt" 2>/dev/null | wc -l)
...
cd ../../autointerp/autointerp_full
if [ -d "results/layer4_top10_qwen7b_financial" ]; then
    rm -rf "results/layer4_top10_qwen7b_financial"
    echo "🗑️  Cleaned up temporary results"
fi
cd ../../InterpUseCases_autointerp/FinanceLabeling

echo "🎯 Layer 4 Top 10 Features Analysis Summary (Qwen 7B Model + Financial Dataset)"
echo "============================================================================="
echo "📊 Results saved in: $RESULTS_DIR/"

# Check final status based on actual results
if [ -d "$RESULTS_DIR/layer4_top10_qwen7b_financial" ] && [ -d "$RESULTS_DIR/layer4_top10_qwen7b_financial/explanations" ]; then
    FEATURE_COUNT=$(find "$RESULTS_DIR/layer4_top10_qwen7b_financial/explanations" -name "layers.4_latent*.txt" 2>/dev/null | wc -l)
    if [ $FEATURE_COUNT -gt 0 ]; then
        echo "   ✅ layer4_top10_qwen7b_financial/ (SUCCESS - $FEATURE_COUNT features analyzed)"
        if [ -f "$RESULTS_DIR/results_summary_layer${LAYER}_qwen7b_financial.csv" ]; then
            echo "      📈 results_summary_layer${LAYER}_qwen7b_financial.csv"
        fi
        ...
        echo "   • results_summary_layer${LAYER}_qwen7b_financial.csv: CSV summary"
        echo ""
...
latents_dir = 'Test_results3/layer4_top10_qwen7b_financial/latents'
log_dir = 'Test_results3/layer4_top10_qwen7b_financial/log'
...
"
        ...
        echo ""
        echo "✅ Layer 4 Top 10 Features analysis with Qwen 7B model + financial dataset completed successfully!"
        echo "🎯 Using Qwen 7B model with Yahoo Finance dataset for faster processing!"
    else
        echo "   ❌ layer4_top10_qwen7b_financial/ (FAILED - no feature explanations found)"
        echo ""
    ...
    echo "   ❌ layer4_top10_qwen7b_financial/ (FAILED - no results directory found)"
    echo ""
